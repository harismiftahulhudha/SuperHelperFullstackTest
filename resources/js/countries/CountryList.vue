<template>
    <div class="p-5">
        <div class="row mb-4">
            ...
            <div class="col-md-6">
                <router-link :to="{ name: 'countries.createOrEdit' }" class="btn btn-primary float-md-right" title="Add New Account">
                    Add New Country
                </router-link>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tr v-for="country, index in data" v-else>
                            <td>{{ country.name }}</td>
                            <td>
                                <router-link class="btn btn-outline-info btn-sm" :to="{name: 'countries.createOrEdit', query: {id: country.id}}" title="Edit">
                                    <i class="fa fa-pencil"></i>
                                </router-link>
                                <button type="button" class="btn btn-outline-danger btn-sm" data-toggle="modal" data-target="#modalDeleteCountry" v-on:click="getModel(index)" title="Delete">
        ...
        <div class="modal fade" id="modalDeleteCountry" tabindex="-1" role="dialog" aria-labelledby="modalDeleteCountryLabel" aria-hidden="true">
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalDeleteCountryLabel">
                            DELETE COUNTRY
                        </h5>
                    ...
                    <div class="modal-body" style="padding-top: 0; padding-bottom: 0;">
                        <p class="text-center">
                            <br>
                            Are you sure want to delete <strong>{{ country.name }}</strong> ?
                        </p>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-success" v-on:click="deleteCountry()" data-dismiss="modal">YES</button>
    ...
    export default {
        ...
        methods: {
            getPagination(page) {
                ...
                this.$router.push({name: "countries.list", query: params})
            },
            searchData() {
                const params = {}
                if (this.query !== '') {
            ...
            deleteCountry() {
                ...
                var url = '/api/countries/' + this.country.id
                axios.post(url, formData, {headers})
                    .then((res) => {
                        toastr.success('Successfully Deleted Country', process.env.MIX_APP_NAME)
